...
export function registerCmdbTools(server: McpServer, client: ServiceNowClient): void {
  ...
  server.registerTool(
    "sn_list_ci",
    {
      ...
      inputSchema: {
        ...
        name: z.string().optional().describe("Filter by name (LIKE)"),
    ...
    async ({ ci_class, query, name, operational_status, limit, offset }) => {
      const parts: string[] = [];
  ...
  server.registerTool(
    "sn_get_ci",
    {
      ...
      inputSchema: {
        sys_id: z.string(),
        ci_class: z.string().default("cmdb_ci"),
      },
    },
