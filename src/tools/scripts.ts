...
export function registerScriptTools(server: McpServer, client: ServiceNowClient): void {

  server.registerTool(
    "sn_list_script_includes",
    {
      ...
      inputSchema: {
        ...
        api_name: z.string().optional().describe("Filter by API name"),
        limit: z.number().int().min(1).max(100).default(20),
        offset: z.number().int().min(0).default(0),
      },
    },
    async ({ query, name, active, api_name, limit, offset }) => {
      const parts: string[] = [];
  ...
  server.registerTool(
    "sn_update_script_include",
    {
      description: "Update an existing script include (commonly used to push script changes).",
      inputSchema: {
        sys_id: z.string(),
        data: z.record(z.string(), z.unknown()).describe("Fields to update (typically { script: '...' })"),
      },
    },
    async ({ sys_id, data }) => {
