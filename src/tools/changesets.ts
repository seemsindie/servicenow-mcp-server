...
export function registerChangesetTools(server: McpServer, client: ServiceNowClient): void {
  ...
  server.registerTool(
    "sn_list_update_sets",
    {
      ...
      inputSchema: {
        ...
        state: z.string().optional().describe("in progress, complete, ignore, etc."),
        limit: z.number().int().min(1).max(100).default(20),
        offset: z.number().int().min(0).default(0),
      },
    },
    async ({ query, state, limit, offset }) => {
      const parts: string[] = [];
  ...
  server.registerTool(
    "sn_create_update_set",
    {
      ...
      inputSchema: {
        ...
        application: z.string().optional().describe("Application sys_id"),
      },
    },
    async (params) => {
  ...
  server.registerTool(
    ...
    async ({ sys_id }) => {
      // Setting current update set is done via user preference
