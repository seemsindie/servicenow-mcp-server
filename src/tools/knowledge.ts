...
export function registerKnowledgeTools(server: McpServer, registry: InstanceRegistry): void {

  server.registerTool("sn_list_knowledge_bases", {
  ...
  server.registerTool("sn_create_knowledge_base", {
    description: "Create a new knowledge base.",
    inputSchema: {
      instance: z.string().optional().describe("Target ServiceNow instance name (from config). Uses default instance if omitted."),
      title: z.string(), description: z.string().optional(), owner: z.string().optional().describe("Owner sys_id"),
    },
  }, async (params) => {
  ...
  server.registerTool("sn_create_kb_category", {
  ...
  server.registerTool("sn_list_articles", {
    description: "List knowledge articles with optional filters.",
    inputSchema: {
      instance: z.string().optional().describe("Target ServiceNow instance name (from config). Uses default instance if omitted."),
      query: z.string().optional(), kb_knowledge_base: z.string().optional(),
      kb_category: z.string().optional(), workflow_state: z.string().optional().describe("e.g. 'published', 'draft', 'retired'"),
      limit: z.number().int().min(1).max(100).default(20), offset: z.number().int().min(0).default(0),
    },
  }, async ({ instance, query, kb_knowledge_base, kb_category, workflow_state, limit, offset }) => {
    const client = registry.resolve(instance);
    const parts: string[] = [];
  ...
  server.registerTool("sn_get_article", {
    description: "Get a knowledge article by sys_id or number.",
    inputSchema: {
      instance: z.string().optional().describe("Target ServiceNow instance name (from config). Uses default instance if omitted."),
      sys_id: z.string().optional(), number: z.string().optional(),
    },
  ...
  server.registerTool("sn_create_article", {
    description: "Create a new knowledge article.",
    inputSchema: {
      instance: z.string().optional().describe("Target ServiceNow instance name (from config). Uses default instance if omitted."),
      short_description: z.string(), text: z.string().describe("Article body (HTML supported)"),
      kb_knowledge_base: z.string().describe("Knowledge base sys_id"),
  ...
  server.registerTool("sn_update_article", {
  ...
  server.registerTool("sn_publish_article", {
