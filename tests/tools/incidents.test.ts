...
import { createMockClient, SAMPLE_INCIDENT, type MockClient } from "../mocks/index.ts";
...
describe("registerIncidentTools", () => {
  ...
  test("registers 7 tools without error", () => {
    registerIncidentTools(server as unknown as McpServer, mockClient as unknown as Parameters<typeof registerIncidentTools>[1]);
  ...
  test("sn_create_incident creates on incident table", async () => {
    ...
    const record = await mockClient.createRecord("incident", {
  ...
  test("sn_resolve_incident sets state to 6", async () => {
    ...
    const record = await mockClient.updateRecord("incident", SAMPLE_INCIDENT["sys_id"] as string, {
      state: "6",
      close_notes: "Fixed by restarting service",
    });

    expect(mockClient._calls.updateRecord).toHaveLength(1);
    expect(mockClient._calls.updateRecord[0]!.body["state"]).toBe("6");
  });

  test("sn_close_incident sets state to 7", async () => {
    registerIncidentTools(server as unknown as McpServer, mockClient as unknown as Parameters<typeof registerIncidentTools>[1]);

    await mockClient.updateRecord("incident", SAMPLE_INCIDENT["sys_id"] as string, {
      state: "7",
      close_code: "Solved (Permanently)",
    });

    expect(mockClient._calls.updateRecord[0]!.body["state"]).toBe("7");
  });

  test("sn_add_incident_comment adds comments field", async () => {
    registerIncidentTools(server as unknown as McpServer, mockClient as unknown as Parameters<typeof registerIncidentTools>[1]);

    await mockClient.updateRecord("incident", "abc123", { comments: "Customer updated" });

    expect(mockClient._calls.updateRecord[0]!.body["comments"]).toBe("Customer updated");
  });

  test("sn_add_incident_work_notes adds work_notes field", async () => {
    registerIncidentTools(server as unknown as McpServer, mockClient as unknown as Parameters<typeof registerIncidentTools>[1]);

    await mockClient.updateRecord("incident", "abc123", { work_notes: "Internal note" });

    expect(mockClient._calls.updateRecord[0]!.body["work_notes"]).toBe("Internal note");
  });
});
